...
- ✅ **Model sweep + baseline + coverage run (2026-07-11, overnight)** → the generator's measured winner is **`gpt-oss:120b`: 82% lint-pass / 0.594 fnRecall / 0.325 fieldExact / 0.204 intent over the full n=60**, beating the deterministic Ledger incumbent (60%), the live qwen (50%), and MiniMax's fragile partial — robust, no crash. Honest reframe: the deterministic tool beats a weak LLM on *structure*; the LLM's value is *intent*; gpt-oss:120b wins both. **101 gpt-oss lint-passing candidates written to Arkiv Braga** (one tx `0x673bca20…`, dataset `erc7730-harvest`, 12.6× the curated 8) + a QLoRA-ready distillation dataset (`harvest-gptoss.jsonl`). Live app stays on qwen (fast UX); gpt-oss:120b is the documented quality ceiling. Numbers + repro → [`erc7730-poc/eval/README.md`](./erc7730-poc/eval/README.md).
- ⏭️ **Deferred:** the QLoRA fine-tune itself (dataset + codex-validated recipe ready; ARM64 env setup risk on an unattended box + low 7B ceiling vs the 82% headline → run deliberately via the NVIDIA Unsloth-on-Spark playbook). MiniMax IQ2_XXS serving fix is moot given gpt-oss:120b.
