...
- **DGX live path written (not run)** — `dgx-wrapper/` (bearer→Ollama qwen3-coder-next→`erc7730 lint`, decimals via eth_call) + Cloudflare-tunnel/ingress/systemd docs; bearer generated (`.secrets/dgx-wrapper-bearer.txt`). `lib/dgx.ts` flips `/api/load`+`/api/generate` to live when `DGX_URL` is set — going live is env + redeploy.
- **DEFERRED (needs the free GPU):** warm qwen3-coder-next + wrapper end-to-end, the eval batch run (real numbers + baseline deltas), the Vercel live swap, codex security+eval audit. The DGX was busy with another job.
