...
changed. The chunk lane (`blob` entities) carries only the measured tail: 40 of 6,119 source
files and 51 of 3,129 metadatas exceed one transaction.
...
| `GET /api/record?address=` | The whole-record VIEW behind tab 2: all 24 fields composed from Arkiv, plus per-field **provenance** (which entities built each field — kind, key, hash, bytes), per-field byte sizes, and the URL of the same field on sourcify.dev and on our route, so every datum is one click from its source on both sides. |
| `GET /api/graph?address=` | One contract as the entities that hold it, as a tree (tab 3): verified_contract → compilation → unique source files (→ blob parts) and the bytecodes it references, with real keys and payload byte breakdowns. Sourcefiles and bytecodes are fetched in batched `or(eq(hash)…)` queries — a 93-file contract costs ~6 reads, not 93. |
| `GET /api/query?...` | The filters Sourcify's public API does not expose: proxy status, compiler version prefix, function-count ranges, optimizer settings — and `hash=` for the content-addressed lanes, which is how the entity browser follows a reference. Returns the literal Arkiv query it ran. |
